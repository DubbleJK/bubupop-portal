...
[제목 titleCandidates 3개 — 눈에 띄게 + SEO]
- 유형: **정보형 / 사례형 / 문제해결형** 각 1개. type 필드에 정확히 이 문자열을 쓴다.
- **메인키워드**는 각 제목에 **반드시 포함**하고, 가능하면 **제목 앞쪽(대괄호 안 또는 첫 12~18자 안)** 에 두어 모바일 검색 결과에서도 키워드가 보이게 한다.
- 형식: 맨 앞에 **대괄호 훅** \`[…]\` + 공백 + 나머지 문구. 대괄호 안에는 **메인키워드 또는 그 축약(띄어쓰기 동일)** + 이익/상황 힌트를 넣는다. 예시 느낌만 참고하고 표절하지 말 것: \`[작업조끼 인쇄]\` \`[부산 스티커 제작]\`
- **클릭을 부르되** 낚시·선정적 과장·허위 약속은 금지. 구체 혜택·상황·궁금증을 한 줄에 담는다.
- **SEO**: 불필요한 특수문자 남발 금지. 읽기 쉬운 한글 위주. 검색 의도(정보/사례/고민해결)가 유형별로 드러나게 문구를 다르게 짠다.
  · 정보형: 알림·가이드·체크 포인트 느낌 + 키워드
  · 사례형: 누가/어떤 상황 + 키워드 + 결과·배움 암시
  · 문제해결형: 흔한 고민 + 키워드 + 해결·팁 암시
- 길이 **26~44자**(공백 포함). 3개 제목이 서로 **겹치지 않게** 다른 각도로 짠다.
...
[재확인]
- 제목 3개: 메인키워드 앞쪽 배치·대괄호 훅·유형별 차별화·26~44자.
